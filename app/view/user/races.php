        ...
        <?php foreach($races as $race) : // Boucle sur chaque course ?>
            ...
            <article class="race-card-home <?= ($nextRace && $race['id'] === $nextRace['id']) ? 'race-next' : '' ?>">
                ...
                <div class="race-card-home-body <?= ($nextRace && $race['id'] === $nextRace['id']) ? 'race-next' : '' ?>">
                    ...
                    <?php if($race['status'] === 'scheduled' && $nextRace && $race['id'] === $nextRace['id']) : ?>
                        <?php if(isset($_SESSION['user_logged'])) : ?>
                            <a href="?action=parier&idRace=<?= $race['id'] ?>" class="btn-connexion">
                                🏁 Parier sur cette course
                            </a>
                        <?php else : ?>
                            <a href="?action=connexion" class="btn-connexion">
